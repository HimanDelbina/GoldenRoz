...
import 'package:nima/Page/ListAmalkard/history_jalasat.dart';
import 'package:shared_preferences/shared_preferences.dart';
import 'package:http/http.dart' as http;

class EditSabt extends StatefulWidget {
  final int? gender;
  final int? id;
  ...
  @override
  _EditSabtState createState() => _EditSabtState();
}

class _EditSabtState extends State<EditSabt> {
  @override
  void initState() {
    print(widget.select);
    getbeoperation();
    ...
    Future.delayed(const Duration(seconds: 2), () {
      for (var i = 0; i < amount[0].length; i++) {
        if (amount[0][i].title.toString() ==
            widget.jalasatPass.amount.toString()) {
          setState(() {
            amountSelect = amount[0][i].id;
            amountPost = amount[0][i].title;
          });
        }
      }
      for (var i = 0; i < tip[0].length; i++) {
        if (tip[0][i].title.toString() == widget.jalasatPass.tip.toString()) {
          setState(() {
            tipSelect = tip[0][i].id;
            tipPost = tip[0][i].title;
          });
        }
      }
      for (var i = 0; i < handpis[0].length; i++) {
        if (handpis[0][i].value == widget.jalasatPass.handpis) {
          setState(() {
            handpisSelect = handpis[0][i].id;
            handpisPost = handpis[0][i].value;
          });
        }
      }
      for (var i = 0; i < device[0].length; i++) {
        if (device[0][i].title.toString() ==
            widget.jalasatPass.device.toString()) {
          setState(() {
            deviceSelect = device[0][i].id;
            devicePost = device[0][i].title;
          });
        }
      }
      for (var i = 0; i < moj[0].length; i++) {
        if (moj[0][i].title.toString() == widget.jalasatPass.moj.toString()) {
          setState(() {
            mojSelect = moj[0][i].id;
            mojPost = moj[0][i].title;
          });
          postdevice();
        }
      }
      // if (pals[0].length != 0) {
      //   for (var i = 0; i < pals[0].length; i++) {
      //     if (pals[0][i].value.toString() ==
      //         widget.jalasatPass.pals.toString()) {
      //       setState(() {
      //         palsSelect = pals[0][i].id;
      //         palsPost = pals[0][i].value;
      //       });
      //     }
      //   }
      // }
    ...
    if (token != null) {
      ...
      if (response.statusCode == 200) {
        ...
        setState(() {
          ...
          amountbool = List.generate(amount[0].length, (bool) => false);
          tipbool = List.generate(tip[0].length, (bool) => false);
          handpisbool = List.generate(handpis[0].length, (bool) => false);
          devicebool = List.generate(device[0].length, (bool) => false);
          mojbool = List.generate(moj[0].length, (bool) => false);
        });
      }
    }
  }
    ...
    if (token != null) {
      ...
      if (response.statusCode == 200) {
        ...
        setState(() {
          ...
          for (var i = 0; i < energy[0].length; i++) {
            if (energy[0][i].value.toString() ==
                widget.jalasatPass.energy.toString()) {
              setState(() {
                energySelect = energy[0][i].id;
                energyPost = energy[0][i].value;
              });
            }
          }
        });
      ...
      if (response.statusCode == 200) {
        ...
        if (x["message"] == "1") {
          Navigator.of(context).push(MaterialPageRoute(
            builder: (context) => HistoryJalasat(
  ...
  //         if (barea![i].area == widget.jalasatPass.area) {
  //           setState(() {
  //             areaFinal = barea![i].id;
